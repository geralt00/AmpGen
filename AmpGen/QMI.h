...
namespace AmpGen{
    namespace QMI{
        ...
        Particle mother(type.decayDescriptor(), type.finalStates());
            ...
            Expression E1 = (fcn::pow(M1, 2) + mother.mass() * mother.mass() - s23)/(2 * mother.mass()); // for the rest frame of the D0 -> input might not be
            ...
            return {s12, s13, s23, theta, p1[0], p1[1], p1[2], p1[3], p2[0], p2[1], p2[2], p2[3], p3[0], p3[1], p3[2], p3[3]};
        }
        ...
        std::vector<Expression> rotateDalitz(EventType& type){
            ...
            real_t m2 = 0.8051636393861085;
             // phi is s12, s13, s23, theta
            std::vector<Expression> Phi(dalitz(type));
            Expression z1 = (Phi[1] + Phi[0])/2; //rotation
            Expression z2 = (Phi[1] - Phi[0])/2;
            Expression w1 = m1 * z1 + c1; //rescaling
            Expression w2 = m2 * z2 + c2;
            // now take w2 and stretch it to be more square
            ...
            // as above but for square dalitz setup
...
//                    w1 = (1/M_PI) * fcn::acos(2 * (z1 - z1Min)/(z1Max - z1Min) - 1);                    
        ...
        // get polynomials returned as strings
        //template <class ce=CompiledExpression<real_t(const real_t*, const real_t*)> >
...
//            
 //       }
        ...
        // map to go from "phasecorrection::CXY" -> product of 2 polynomials order X and Y
        template <class ce=CompiledExpression<real_t(const real_t*, const real_t*)> >
        std::map<std::string, ce> cPhaseCorrection(EventType& type, MinuitParameterSet& MPS){
            ...
            if (polyType=="Gaussian2"){
                ...
                for (size_t i=0;i<O+1;i++){ // NOTE this is O for the Order not a 0 zero!!
...
//        if (s01>s02) g = A*exp_pp * exp_mm;
       ...
       // if (s01<s02) g = -A*exp_pm * exp_mp;
        ...
        // gives a real number from an event, the map and the parameters
        template <class ce=CompiledExpression<real_t(const real_t*, const real_t*)> >
        real_t phaseCorrection(const Event& evt, std::map<std::string, ce>& cPij, MinuitParameterSet& MPS){
            ...
            if (NamedParameter<std::string>("PhaseCorrection::PolyType", "antiSym_legendre")  == "Gaussian" ){
               ...
               rt = Gauss2Dalitz(evt, MPS);
                ...
                for (auto& p : cPij){
                    real_t temp = MPS[p.first]->mean() * p.second(evt.address());
                    INFO("coeff is " << MPS[p.first]->mean() << " correction term  " << temp);
                    rt += temp;
                }
            } // supplying the key from the map to the MPS where it also exists
           return rt; 
...
//            return std::arg(std::conj(A.getValNoCache(evt)) * Abar.getValNoCache(evt)) ;
        ...
        // lost of options of input params for when calculating cos and sin
        //template <class ce=CompiledExpression<real_t,real_t*,real_t*> >
        template <class ce=CompiledExpression<real_t(const real_t*, const real_t*)> >
        ...
        //most likely unused this one below
        template <class ce=CompiledExpression<real_t(const real_t*, const real_t*)> >
        real_t cosTerm(std::map<std::string, std::vector<real_t> >* amps, MinuitParameterSet& MPS, std::map<std::string, ce>& cPij, EventList_type& mcSig){ 
            real_t C=0;
           // INFO("Starting CosTerm Function, before OMP loop");
        ...
        template <class ce=CompiledExpression<real_t,real_t*,real_t*> >// this is not integrated though??
        ...
        real_t probCKM_unnorm_Int(const Event& evt, CoherentSum& A, CoherentSum& Abar, MinuitParameterSet& MPS, std::map<std::string, ce>& cPij, int gammaSign){
            ...
            // 6/2/22 changed the sign next to gammasign above
        }
 ...
 // this one above, NOT the first one for probCorr_unnorm
        template <class ce=CompiledExpression<real_t,real_t*,real_t*> >
...
// commented out 11/1 ue to error on lxplus: "expected primary-expression before ‘(’ token"
...
//            if (boostEvents){
        ...
        void writeDalitz(EventList_type& list, const std::string& name="temp"){
            ...
            TTree t(("Dalitz_"+name).c_str(), ("Dalitz_"+ name).c_str());
