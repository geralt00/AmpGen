...
int main(int argc, char * argv[]){
  ...
  //******* PARSE THE ARGS FROM COMMAND LINE AND THE OPTIONS FILE*********
	...
	if (normalisationTuple_DK.empty()){
		...
		ArgumentPack args;
		eventsMC.loadFromFile( normalisationTuple_DK+":DalitzEventList" , args );
	...
	auto LL_DK = [&events_dk_Bminus, &events_dk_Bplus, &eventsMC, 
		...
		for (size_t i=0; i < events_dk_Bminus.size(); i++)
		{
			...
			ll_running += log(1.*(probability/normalisation_Bminus));
		...
		for (size_t i=0; i < events_dk_Bplus.size(); i++)
		{
			...
			ll_running += log(1.*(probability/normalisation_Bplus) );
		}
