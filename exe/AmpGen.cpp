...
// added 9/3, can remove if not writing amplitudes
...
int main( int argc, char** argv )
{
  ...
  */
  ...
  //****** temp added 9/3 to get amplitudes written into tuple for ci si calculation:
  // MinuitParameterSet MPS_Kspipi;
  // MPS_Kspipi.loadFromFile("../Kspipi.opt");
  // EventType plainEventType{NamedParameter<std::string>("PlainEventType", "", "Signal Type to generate, should be for D0 -> Ks0pi-pi+")};
  // CoherentSum A(plainEventType, MPS_Kspipi);
  // CoherentSum Abar(plainEventType.conj(true), MPS_Kspipi);
  // A.prepare(); Abar.prepare();
  // writeAmplitudes(A, Abar, accepted);
  //******
