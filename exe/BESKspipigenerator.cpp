...
int main(int argc, char *argv[])
{
	...
	const bool plot_phase = NamedParameter<bool>("plot_phase", false, "plot the phase of the amplitude");
	...
	//**************  CP:
	int CPsign;
	auto totalA_CP = [&tKSpipi, &cPhi, &deltaCorrection, &CPsign](Event event)
	{
		return totalAmplitudeSquared_CP_KSpipi(CPsign, tKSpipi, cPhi, deltaCorrection, event);
	};
	...
	auto totalA_flavour = [&isFlavour, &tKSpipi, &cPhi](Event event)
	{
		return totalAmplitudeSquared_flavour_KSpipi(isFlavour, tKSpipi, cPhi, event);
	};
	...
	//************* SAME (Kspipi tag):
	auto totalA_same = [&tKSpipi, &cPhi, &deltaCorrection](Event event_main, Event event_tag)
	{
		return totalAmplitudeSquared_BES_KSpipi(tKSpipi, cPhi, deltaCorrection, event_main, event_tag);
	};
	...
	if (plot_phase)
	{
		auto my_dd = [&tKSpipi, &cPhi](Event &event)
		{
			...
			complex<double> A = tKSpipi.Amp_PFT(p1, p2, p3);
			...
			complex<double> Abar = tKSpipi.Amp_PFT(reverse_p1, reverse_p3, reverse_p2);
			return (arg(A*conj(Abar)));
		};
		...
		auto amp_A = [&tKSpipi, &cPhi](Event &event)
		{
			vector<double> p1, p2, p3;
			for (int i = 0; i < 4; i++)
			{
