...
int main(int argc, char * argv[]){
	...
	const size_t      nInt = NamedParameter<size_t>("nInt", 10000000, "Total number of events to generate" );
...
// loop over nInt / blocksize times through generating and summing blocksize events
	...
	//******* PREPARE LAMBDAS TO FILL THE THINGS ******
		auto fillSums = [ &tKSpipi, &cPhi](Event& event, int& binNumber, real_t& correction, RealVec_t& FSums, RealVec_t& FbarSums, RealVec_t& cSums, RealVec_t& sSums)
		{
			...
			complex<double> complexA = tKSpipi.Amp_PFT(p1, p2, p3);
			...
			complex<double> complexAbar = tKSpipi.Amp_PFT(reverse_p1, reverse_p3, reverse_p2);
			real_t modA{ std::abs(complexA) };
			real_t modAbar{ std::abs(complexAbar) };
			real_t totalPhase { std::arg((complexA)*conj(complexAbar))+ correction};
