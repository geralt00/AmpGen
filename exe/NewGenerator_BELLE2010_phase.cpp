...
int main(int argc , char* argv[] ){
    ...
    for(unsigned i=0; i<Binfo.nTypes; i++)
	{
		...
		auto AtotalCorrected = [&A, &Abar, &amp, &cPhi, &phaseCorrection, &MPS, &Bsign](Event event){
			real_t deltaC{0};
			...
			return totalAmplitudeSquared_rB_Belle2010(Bsign, A, Abar, *amp, cPhi, deltaC, MPS,event);//Here is the problem I set amplitude squared from 2018 as the pdf
		};
