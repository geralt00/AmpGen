...
int main(int argc, char * argv[]){
  ...
  //******* PARSE THE ARGS FROM COMMAND LINE AND THE OPTIONS FILE*********
	...
	//******* LOAD THE DATA ******
	EventList_type events_dk_Bminus((inputData_DK + ":Bminus_DalitzEventList").c_str() , signalType);
	EventList_type events_dk_Bplus((inputData_DK + ":Bplus_DalitzEventList").c_str() , signalType);
	EventList_type events_dpi_Bminus((inputData_DPi + ":Bminus_DalitzEventList").c_str() , signalType);
	EventList_type events_dpi_Bplus((inputData_DPi + ":Bplus_DalitzEventList").c_str() , signalType);
	...
	auto LL_DK = [&events_dk_Bminus, &events_dk_Bplus, &eventsMC, 
		...
		real_t normalisation_Bminus = totalAmplitudeSquared_Integrated(signs.BminusSign, normA, normAbar, MPS, normalisationCrossTerms_m);
		real_t normalisation_DPi_Bplus = totalAmplitudeSquared_DPi_Integrated(signs.BplusSign, normA, normAbar, MPS, normalisationCrossTerms_p);
		real_t normalisation_DPi_Bminus = totalAmplitudeSquared_DPi_Integrated(signs.BminusSign, normA, normAbar, MPS, normalisationCrossTerms_m);
		real_t  ll_running{0};
		...
		for (size_t i=0; i < events_dk_Bminus.size(); i++)
		{
			...
			real_t probability_DPi{totalAmplitudeSquared_DPi_XY(signs.BminusSign, amplitudes_dk_Bminus.A[i], amplitudes_dk_Bminus.Abar[i], amplitudes_dk_Bminus.deltaD[i], correction, MPS) };
			ll_running += log(1.*(probability/normalisation_Bminus) + 0.0*(probability_DPi/normalisation_DPi_Bminus));
		...
		for (size_t i=0; i < events_dk_Bplus.size(); i++)
		{
			...
			real_t probability_DPi{totalAmplitudeSquared_DPi_XY(signs.BplusSign, amplitudes_dk_Bplus.A[i], amplitudes_dk_Bplus.Abar[i], amplitudes_dk_Bplus.deltaD[i], correction, MPS) };
			ll_running += log(1.*(probability/normalisation_Bplus) + 0.0*(probability_DPi/normalisation_DPi_Bplus));
		}
	...
	auto LL_DPi = [&events_dpi_Bminus, &events_dpi_Bplus, &eventsMC, 
		...
		real_t normalisation_DK_Bplus = totalAmplitudeSquared_Integrated(signs.BplusSign, normA, normAbar, MPS, normalisationCrossTerms_p);
		real_t normalisation_DK_Bminus = totalAmplitudeSquared_Integrated(signs.BminusSign, normA, normAbar, MPS, normalisationCrossTerms_m);
		real_t  ll_running{0};
		#pragma omp parallel for reduction (+:ll_running)
		...
		for (size_t i=0; i < events_dpi_Bminus.size(); i++)
		{
			...
			real_t probability_DK{totalAmplitudeSquared_XY(signs.BminusSign, amplitudes_dpi_Bminus.A[i], amplitudes_dpi_Bminus.Abar[i], amplitudes_dpi_Bminus.deltaD[i], correction, MPS) };
			ll_running += log(1.0*(probability/normalisation_Bminus) + 0.00*(probability_DK/normalisation_DK_Bminus));
		...
		for (size_t i=0; i < events_dpi_Bplus.size(); i++)
		{
			...
			real_t probability_DK{totalAmplitudeSquared_XY(signs.BplusSign, amplitudes_dpi_Bplus.A[i], amplitudes_dpi_Bplus.Abar[i], amplitudes_dpi_Bplus.deltaD[i], correction, MPS) };
			ll_running += log(1.0*(probability/normalisation_Bplus) + 0.00*(probability_DK/normalisation_DK_Bplus));
		}
