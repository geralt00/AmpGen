...
/*
	auto dk_DD_sWeight_norm = [&events_Bminus_DD, &events_Bplus_DD]()
	{
		...
		auto dk_LL_sWeight_norm = [&events_Bminus_LL, &events_Bplus_LL]()
	{
		std::vector<double_t> alpha{0, 0, 0, 0};
		double_t  sw{0};
		double_t sw_sq{0};
	...
	*/
	...
	auto NLL_LL = [&events_Bminus_LL, &events_Bplus_LL, &eventsMC, 
					 ...
					 &normA, &normAbar, &MPS,
					 &phaseCorrection, &signs, &alpha_LL](){
		...
		for (size_t i=0; i < events_Bminus_LL.size(); i++)
		{
			...
			ll_running += events_Bminus_LL[i].weight_bkg() * log(probability/normalisation_Bminus);		
				
			//ll_running_m += (events_Bminus_LL[i].weight_bkg() * log(probability));
		...
		for (size_t i=0; i < events_Bplus_LL.size(); i++)
		{
			...
			//ll_running += events_Bplus_LL[i].weight_bkg() * log(probability/normalisation_Bplus);
			//ll_running_p += (events_Bplus_LL[i].weight_bkg() * log(probability));
		}

		//return  -2*alpha_LL[1]*(ll_running_m - 2033*log(normalisation_Bminus)) -2*alpha_LL[0]*(ll_running_p - 2033*log(normalisation_Bplus));
		return -2*alpha_LL* (ll_running);
	};

	auto NLL_DD = [&events_Bminus_DD, &events_Bplus_DD, &eventsMC, 
					 &amplitudesMC, &amplitudes_Bminus_DD, &amplitudes_Bplus_DD, &nEvents,
					 &normA, &normAbar, &MPS,
					 &phaseCorrection, &signs, &alpha_DD](){
		...
		for (size_t i=0; i < events_Bminus_DD.size(); i++)
		{
			...
			//ll_running += (events_Bminus_DD[i].weight_bkg() * log(probability/normalisation_Bminus));		
			ll_running += events_Bminus_DD[i].weight_bkg() * log(probability/normalisation_Bminus);	
		...
		for (size_t i=0; i < events_Bplus_DD.size(); i++)
		{
			...
			//ll_running += (events_Bplus_DD[i].weight_bkg() * log(probability/normalisation_Bplus));
			ll_running += events_Bplus_DD[i].weight_bkg() * log(probability/normalisation_Bplus);
		}
		return -2*alpha_DD* (ll_running);
