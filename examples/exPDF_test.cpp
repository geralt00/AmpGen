...
//  } 
  ...
  eventsMC.emplace_back( datasets[1], evtType, Branches(MCbNames), GetGenPdf(true), WeightBranch(mc_weight_branch) );
  ...
  auto evalA_fav = sig_fav.amplitudeEvaluator(&events[1]);
  ...
  for (size_t i=0; i < events[0].size(); i++){
                ...
                complex_t thisA_fav = evalA_fav(events[1][i]);
		real_t probabilitysig = corrected_pdfsig(thisA,thisA_fav,5.867,190,1,MPS);
  ...
  auto evalA_bar = sig.amplitudeEvaluator(&events[0]);
        for (size_t i=0; i < nEvents; i++){
                complex_t thisA = evalA(events[1][i]);
		complex_t thisA_bar = evalA_bar(events[0][i]);
                //real_t probabilitysig = pow(abs(thisA),2);
                real_t probabilitysig = corrected_pdfsig(thisA,thisA_bar,5.867,190,1,MPS);
