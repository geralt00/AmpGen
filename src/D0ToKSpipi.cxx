...
void D0ToKSpipi::init(){
  //std::cout << "D0ToKSpipi ==> Initialization !" << std::endl;

  _nd = 3;
  tan2thetaC = (0.22650*0.22650)/(1.-(0.22650*0.22650)) ;  //sin(theta_C) = 0.22650 +/- 0.00048
  pi180inv = 1.0*3.1415926/180;

  mass_R[0]=   0.77526; width_R[0]=  0.14740; spin_R[0]=  1; ar[0]=  1;        phir[0]=  0;
  mass_R[1]=   0.78266; width_R[1]=  0.00868; spin_R[1]=  1; ar[1]=  0.037606; phir[1]=  109.677;
  mass_R[2]=   1.27550; width_R[2]=  0.18670; spin_R[2]=  2; ar[2]=  1.54909;  phir[2]=  -42.7425;
  mass_R[3]=   1.46500; width_R[3]=  0.40000; spin_R[3]=  1; ar[3]=  3.70735;  phir[3]=  103.644;
  mass_R[4]=   0.89167; width_R[4]=  0.0514;  spin_R[4]=  1; ar[4]=  1.86093;  phir[4]=  136.529;
  mass_R[5]=   1.42730; width_R[5]=  0.10000; spin_R[5]=  2; ar[5]=  1.74288;  phir[5]=  -48.0968;
  mass_R[6]=   1.71800; width_R[6]=  0.3220;  spin_R[6]=  1; ar[6]=  3.31;     phir[6]=  -118.2;
  mass_R[7]=   1.41400; width_R[7]=  0.2320;  spin_R[7]=  1; ar[7]=  0.171672; phir[7]=  -68.41;
  mass_R[8]=   0.89167; width_R[8]=  0.0514;  spin_R[8]=  1; ar[8]=  0.164;    phir[8]=  -42.2;
  mass_R[9]=   1.42730; width_R[9]=  0.1000;  spin_R[9]=  2; ar[9]=  0.1;      phir[9]=  -89.6;
  mass_R[10]=  1.41400; width_R[10]= 0.2320;  spin_R[10]= 1; ar[10]= 0.21;     phir[10]= 150.2;
  mass_R[11]=  1.42500; width_R[11]= 0.2700;  spin_R[11]= 1; ar[11]= 2.78276;  phir[11]= 97.9608;
  mass_R[12]=  1.42500; width_R[12]= 0.2700;  spin_R[12]= 1; ar[12]= 0.11;     phir[12]= 162.3;

  beta[0] = complex<double>( 0.255303*cos( 47.8861 *pi180inv),  0.255303*sin( 47.8861 *pi180inv));
  beta[1] = complex<double>(13.4446  *cos( -5.11127*pi180inv), 13.4446  *sin( -5.11127*pi180inv));
  beta[2] = complex<double>(38.8496  *cos(-30.06   *pi180inv), 38.8496  *sin(-30.06   *pi180inv));
  beta[3] = complex<double>(13.1086  *cos(-81.4148 *pi180inv), 13.1086  *sin(-81.4148 *pi180inv));
  beta[4] = complex<double>(0., 0.);

  fprod[0] = complex<double>(5.08049*cos(-182.312*pi180inv), 5.08049*sin(-182.312*pi180inv));
  fprod[1] = complex<double>(17.2388*cos(-219.209*pi180inv), 17.2388*sin(-219.209*pi180inv));
  fprod[2] = complex<double>(19.0145*cos(-76.9884*pi180inv), 19.0145*sin(-76.9884*pi180inv));
  fprod[3] = complex<double>(11.9875*cos(-190.502*pi180inv), 11.9875*sin(-190.502*pi180inv));
...
complex<double> D0ToKSpipi::Amp_PFT(vector<double> k0l, vector<double> pip, vector<double> pim) {
...
complex<double> D0ToKSpipi::Resonance2(vector<double> p4_p, vector<double> p4_d1, vector<double> p4_d2, double mag, double theta, double gamma, double bwm, int spin) {
  ...
  switch (spin) {
    ...
    case 2:
      ...
      break;
    default:
      cout << "Incorrect spin in D0ToKSpipi::EvtResonance2.cc\n" <<endl;
  }
...
complex<double> D0ToKSpipi::K_matrix(vector<double> p_pip, vector<double> p_pim) { 
...
complex<double> D0ToKSpipi::amplitude_LASS(vector<double> p_k0l, vector<double> p_pip, vector<double> p_pim, string reso, double A_r, double Phi_r) {
  double mR = 1.425 ;
  double gammaR = 0.27 ;
  double mab2 = 0.0;
	...
	TLorentzVector _p_pip(p_pip[0],p_pip[1],p_pip[2],p_pip[3]);
	TLorentzVector _p_pim(p_pim[0],p_pim[1],p_pim[2],p_pim[3]);
  if     (reso == "k0spim") mab2 = pow((_p_k0l + _p_pim).Mag(),2);
  else if(reso == "k0spip") mab2 = pow((_p_k0l + _p_pip).Mag(),2);
  double s = mab2;
