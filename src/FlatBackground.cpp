...
FlatBackgroundPdf::FlatBackgroundPdf() = default; 

FlatBackgroundPdf::FlatBackgroundPdf( const EventType& type, const MinuitParameterSet& mps, const std::string& prefix )
  :   m_evtType  (type)
      , m_printFreq(NamedParameter<size_t>(     "FlatBackgroundPdf::PrintFrequency", 100)  )
      , m_dbThis   (NamedParameter<bool>(       "FlatBackgroundPdf::Debug"         , false))
      , m_verbosity(NamedParameter<bool>(       "FlatBackgroundPdf::Verbosity"     , 0)    )
  , m_objCache (NamedParameter<std::string>("FlatBackgroundPdf::ObjectCache"   ,"")    )
  , m_prefix   (prefix)
      , m_mps(&mps) 
{
}

void FlatBackgroundPdf::prepare()
{
}

void FlatBackgroundPdf::updateNorms()
{
}

void FlatBackgroundPdf::debug( const Event& evt, const std::string& nameMustContain )
{
}

std::vector<FitFraction> FlatBackgroundPdf::fitFractions(const LinearErrorPropagator& linProp)
{
  std::vector<FitFraction> outputFractions;
  return outputFractions;
}

complex_t FlatBackgroundPdf::getValNoCache( const Event& evt ) const
{
  std::complex<double> value(0,0);
  return value; 
}

void FlatBackgroundPdf::reset( bool resetEvents )
{
  m_prepareCalls                                     = 0;
  m_lastPrint                                        = 0;
  if ( resetEvents ){ 
...
void FlatBackgroundPdf::setEvents( const EventList_type& list )
{
...
void FlatBackgroundPdf::setMC( const EventList_type& sim )
{
...
real_t FlatBackgroundPdf::norm() const
{
...
real_t FlatBackgroundPdf::norm(const Bilinears& norms) const
{
  complex_t acc(0, 0);
...
complex_t FlatBackgroundPdf::norm(const size_t& x, const size_t& y) const
{
  return m_normalisations.get(x, y);
}

void FlatBackgroundPdf::transferParameters()
{
  //for ( auto& mE : m_matrixElements ) mE.coefficient = mE.coupling();
  //m_weight.update();
}

void FlatBackgroundPdf::printVal(const Event& evt)
{
  /*
...
complex_t FlatBackgroundPdf::getVal( const Event& evt ) const
{
  complex_v value( 0., 0. );
...
real_v FlatBackgroundPdf::operator()( const real_v* /*evt*/, const unsigned block ) const 
{
  complex_v value( 0., 0. );
...
std::function<real_t(const Event&)> FlatBackgroundPdf::evaluator(const EventList_type* ievents) const 
{
  ...
  {
    utils::store( values.data() + block * utils::size<real_v>::value,  (m_weight * events->flatbkgPDF(block))  );
  }
  return arrayToFunctor<real_t, typename EventList_type::value_type>(values);
}

std::function<complex_t(const Event&)> FlatBackgroundPdf::amplitudeEvaluator(const EventList_type* ievents) const 
{
  auto events = ievents == nullptr ? m_integrator.events<EventList_type>() : ievents;  
  FunctionCache<EventList_type, complex_v, Alignment::AoS> store(events, m_matrixElements);
  for( auto& me : m_matrixElements ) store.update( me );
...
FlatBackgroundPdf::~FlatBackgroundPdf()
