...
PhaseCorrection_KLpipi::PhaseCorrection_KLpipi()
    :   order_{NamedParameter<size_t>("PhaseCorrection_KLpipi::Order", 0, "Order of polynomial of phase correction")} , 
        correctionType_{NamedParameter<std::string>("PhaseCorrection_KLpipi::CorrectionType", "antiSym_legendre")}
{
    ...
    if (correctionType_ == "singleBias" || correctionType_ == "doubleBias"){
        ...
        for (size_t i{0}; i < nBias_; i++){
            mu_[0].plus = NamedParameter<real_t>("PhaseCorrection_KLpipi::biasParam"+std::to_string(i+1)+":mu+", 0);
            mu_[0].minus = NamedParameter<real_t>("PhaseCorrection_KLpipi::biasParam"+std::to_string(i+1)+":mu-", 0);
            sigma_[0].plus = NamedParameter<real_t>("PhaseCorrection_KLpipi::biasParam"+std::to_string(i+1)+":sigma+", 0);
            sigma_[0].minus = NamedParameter<real_t>("PhaseCorrection_KLpipi::biasParam"+std::to_string(i+1)+":sigma-", 0);
            epsilon_[0] = NamedParameter<real_t>("PhaseCorrection_KLpipi::biasParam"+std::to_string(i+1)+":epsilon", 0);
            A_[0] = NamedParameter<real_t>("PhaseCorrection_KLpipi::biasParam"+std::to_string(i+1)+":A", 0);
            nBias_ = 1;
        }
        doBias_ = true;
    }else{
        ...
        for (size_t i{0}; i < order_ ; i++){
            for (size_t j{1} ; j < order_-i+1 ; j+=2){
                coefficients_.push_back(NamedParameter<real_t>("PhaseCorrection_KLpipi::C_"+std::to_string(i) + "_" + std::to_string(j), 0));
...
PhaseCorrection_KLpipi::PhaseCorrection_KLpipi(MinuitParameterSet &MPS)
    :   order_{NamedParameter<size_t>("PhaseCorrection_KLpipi::Order", 0, "Order of polynomial of phase correction")} , 
        correctionType_{NamedParameter<std::string>("PhaseCorrection_KLpipi::CorrectionType", "antiSym_legendre")}
{
    ...
    if (correctionType_ == "singleBias" || correctionType_ == "doubleBias"){
        ...
        for (size_t i{0}; i < nBias_; i++){
            mu_[i].plus = MPS["PhaseCorrection_KLpipi::biasParam"+std::to_string(i+1)+":mu+"]->mean();
            mu_[i].minus = MPS["PhaseCorrection_KLpipi::biasParam"+std::to_string(i+1)+":mu-"]->mean();
            sigma_[i].plus  = MPS["PhaseCorrection_KLpipi::biasParam"+std::to_string(i+1)+":sigma+"]->mean();
            sigma_[i].minus  = MPS["PhaseCorrection_KLpipi::biasParam"+std::to_string(i+1)+":sigma-"]->mean();
            epsilon_[i]  = MPS["PhaseCorrection_KLpipi::biasParam"+std::to_string(i+1)+":epsilon"]->mean();
            A_[i] = MPS["PhaseCorrection_KLpipi::biasParam"+std::to_string(i+1)+":A"]->mean();
        }
        doBias_ = true;
    }else{
        if (correctionType_ != "antiSym_legendre"){
        ...
        for (size_t i{0}; i < order_ ; i++){
            for (size_t j{1} ; j < order_-i+1 ; j+=2){
                coefficients_.push_back(MPS["PhaseCorrection_KLpipi::C_" + std::to_string(i) + "_" + std::to_string(j)]->mean());
                iIndices_.push_back(i); jIndices_.push_back(j);
            }
        }
        nTerms_ = coefficients_.size();
...
dalitzPair<Expression> PhaseCorrection_KLpipi::transformedDalitzCoords(EventType& eventType)
{
    ...
    if (NamedParameter<bool>("PhaseCorrection_KLpipi::stretchAntiSym", true)){
        auto antiSym_scale = Expression(NamedParameter<real_t>("PhaseCorrection_KLpipi::stretchAntiSym_scale", 2));
        auto antiSym_offset = Expression(NamedParameter<real_t>("PhaseCorrection_KLpipi::stretchAntiSym_offset", 2));
...
dalitzPair<Expression> PhaseCorrection_KLpipi::squareDalitzCoords(EventType& eventType)
{
...
Expression PhaseCorrection_KLpipi::legendre(Expression& x, real_t n)
{
    if (n==0) return 1;
    if (n==1) return x;
...
real_t PhaseCorrection_KLpipi::gaussianExponent(real_t& s, real_t& mu, real_t& sigma)
{
    return std::pow( (s-mu)/sigma , 2);
}

real_t PhaseCorrection_KLpipi::bias(dalitzPair<real_t>& coords, size_t& index)
{
...
Expression PhaseCorrection_KLpipi::polynomial(EventType& eventType, size_t i, size_t j)
{
...
void PhaseCorrection_KLpipi::compilePolynomialExpressions(EventType& eventType)
{
...
real_t PhaseCorrection_KLpipi::eval(const Event& event)
{
    real_t deltaC{0};
...
real_t PhaseCorrection_KLpipi::evalBias(Event& event)
{
    real_t deltaC{0};
...
void PhaseCorrection_KLpipi::updateCoeffs(MinuitParameterSet& MPS)
{   
    size_t n{0};
    for (size_t i{0}; i < order_ ; i++){
        for (size_t j{1} ; j < order_-i+1 ; j+=2){
            coefficients_[n] = (MPS["PhaseCorrection_KLpipi::C_" + std::to_string(i) + "_" + std::to_string(j)]->mean());
            n++;
        }
    }
...
bool PhaseCorrection_KLpipi::doPolynomial()
{
    return doPolynomial_;
}
bool PhaseCorrection_KLpipi::doBias()
{
    return doBias_;
}
